package com.jeans.cosmetic_project.common.paging;
...
public class Pagenation {
    ...
    /* 검색어가 존재하는 경우 검색 조건으로 select 후 페이징 처리를 하기 위한 용도(계산이 들어가 있음) */
    public static SelectCriteria getSelectCriteria(int page, int totalCount, int limit, int buttonAmount, Map<String, String> searchMap) {
        ...
        /* 총 페이지수 계산 */
        /*ex) totalCount:123 limit : 10일때 나눠서 나온 값을 반올림 하겠다. 그러면 총 13page*/
        maxPage = (int) Math.ceil((double) totalCount / limit);

        /* 현재 페이지에 보여줄 시작 페이지 수 */
        /*1<<>>>5
         * page : 현재 요청하는 page ex)8 그러면 start는 6 buttonAmount : 5*/
